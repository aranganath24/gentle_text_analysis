...
**Alternatively**, if you are familiar with the concept of file paths, and know the file path to the folder containing the downloaded Diario "creatives" text files, you can set the working directly using the ```setwd()``` function, where the argument to the function is the relevant file path enclosed in quotation marks. For example, the following code sets the working directory to the folder containing the creatives from the El Diario corpus: 
...
# "diario_creatives_corpus" corpus into a tidy data frame, where each file's 
...
*Please note that data frames which are reproduced in this tutorial are sometimes truncated, to preserve memory; you will therefore see fewer rows in data frames within the tutorial material than you will see when inspecting those same data frames within the RStudio data viewer on your computers*
...
Alternatively, if you want to see all of the stop words within ```stop_words```, you can view it within the RStudio data viewer (i.e. ```View(stop_words)```).
...
To see the plot we just created, we can print the name of the object to which it has been assigned, ```diario_frequency_graph```, within our console; it will appear in the "Plots" tab on the bottom-right of your RStudio interface. 
